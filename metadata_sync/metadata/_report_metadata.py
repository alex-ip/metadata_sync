...
class ReportMetadata(Metadata):
    ...
    _filename_pattern = 'report\.txt$'
    _orient_map = {'NUP': 0.0, 'EUP': 90.0, 'SUP': 180.0, 'WUP': -90.0}

    # Define template text for output. N.B: All format specifiers are for
    # strings
    _template_text = """
...
"""
    ...
    def __init__(self, source=None):
        ...
        """
        self._value_dict = ReportMetadata._template_dict.copy(
    ...
    def parse_report_string(self, report_string, tree_dict=None):
        ...
        def get_name_value(input_text):
            ...
            """
            m = re.match('^\W*(.*?):\s+(.*?)\s*-*$',
                         input_text)  # Look for "name: value" expression
            if m and len(m.groups()) == 2:
                result = m.groups()
                # Remove any non-alphanumeric characters from name and convert
                # to upper case
